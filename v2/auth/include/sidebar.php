<div class="left-side-menu">

    <!-- LOGO -->
    <a href="#" class="logo text-center logo-light">
                    <span class="logo-lg">
                        <img src="../assets/images/logo.png" alt="" height="16">
                    </span>
        <span class="logo-sm">
                        <img src="../assets/images/logo_sm.png" alt="" height="16">
                    </span>
    </a>

    <!-- LOGO -->
    <a href="#" class="logo text-center logo-dark">
                    <span class="logo-lg">
                        <img src="../assets/images/logo-dark.png" alt="" height="16">
                    </span>
        <span class="logo-sm">
                        <img src="../assets/images/logo_sm_dark.png" alt="" height="16">
                    </span>
    </a>

    <div class="h-100" id="left-side-menu-container" data-simplebar>

        <!--- Sidemenu -->
        <ul class="metismenu side-nav">
            <li class="side-nav-title side-nav-item">Navigation</li>

            <li class="side-nav-item">
                <a href="home.php" class="side-nav-link">
                    <i class="uil-dashboard"></i>
                    <span> Dashboard </span>
                </a>
            </li>

            <li class="side-nav-item">
                <a href="javascript: void(0);" class="side-nav-link">
                    <i class="uil-store-alt"></i>
                    <span> Milk Production </span>
                    <span class="menu-arrow"></span>
                </a>
                <ul class="side-nav-second-level" aria-expanded="false">
                    <li>
                        <a href="milk-collection-view-new.php">View New Collection</a>
                        <a href="milk-collection-create.php">Create New Collection</a>
                        <a href="milk-collection-search.php">Search Milk Collection</a>
                        <a href="milk-collection-view.php">View Milk Collection</a>
                        <a href="milk-sales-view.php">View Sales</a>
                        <a href="milk-sales-create.php">Create Sales</a>
                    </li>
                </ul>
            </li>

            <li class="side-nav-item">
                <a href="javascript: void(0);" class="side-nav-link">
                    <i class="uil-crockery"></i>
                    <span> Cow Feed </span>
                    <span class="menu-arrow"></span>
                </a>
                <ul class="side-nav-second-level" aria-expanded="false">
                    <li>
                        <a href="feed-view.php">View</a>
                        <a href="feed-create.php">Create</a>
                        <a href="feed-search.php">Search</a>
                    </li>
                </ul>
            </li>

            <li class="side-nav-item">
                <a href="javascript: void(0);" class="side-nav-link">
                    <i class="uil-syringe"></i>
                    <span> Vaccine </span>
                    <span class="menu-arrow"></span>
                </a>
                <ul class="side-nav-second-level" aria-expanded="false">
                    <li>
                        <a href="vaccine-view.php">View</a>
                        <a href="vaccine-create.php">Create</a>
                        <a href="vaccine-search.php">Search</a>
                    </li>
                </ul>
            </li>

            <li class="side-nav-item">
                <a href="javascript: void(0);" class="side-nav-link">
                    <i class="uil-chart"></i>
                    <span> Report </span>
                    <span class="menu-arrow"></span>
                </a>
                <ul class="side-nav-second-level" aria-expanded="false">
                    <li>
                        <a href="report-expenses.php">Expenses</a>
                        <a href="report-sale.php">Milk Sales</a>
                    </li>
                </ul>
            </li>


            <?php if($_SESSION['user']['usertype'] == 'admin'){ ?>

            <li class="side-nav-item">
                <a href="javascript: void(0);" class="side-nav-link">
                    <i class="uil-user-check"></i>
                    <span> User </span>
                    <span class="menu-arrow"></span>
                </a>
                <ul class="side-nav-second-level" aria-expanded="false">
                    <li>
                        <a href="user-view.php">View User</a>
                        <a href="user-create.php">Create User</a>
                    </li>
                </ul>
            </li>

            <?php } ?>


        </ul>

        <div class="clearfix"></div>

    </div>
    <!-- Sidebar -left -->

</div>